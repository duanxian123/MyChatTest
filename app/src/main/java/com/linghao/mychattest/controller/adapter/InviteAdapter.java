...
/**
 * Created by linghao on 2017/7/24.
 * QQ：782695971
 * 作用：邀请信息listview适配器
 */
...
//            mInvitationInfos.addAll(invationInfos);
            for (int i=0;i<invationInfos.size();i++){
                mInvitationInfos.add(invationInfos.get(i));
            }
            notifyDataSetChanged();
            for(int i=0;i<invationInfos.size();i++){
                Log.e("invations",invationInfos.get(i).toString()+"-----"+i+"---");
            }
        }
    }
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        // 2 获取当前item数据
        final InvationInfo  invationInfo = mInvitationInfos.get(position);
        Log.e("dfsdfsd",invationInfo.toString()+"-----"+position+"---");
//        hodler.accept.setOnClickListener(new View.OnClickListener() {
//            @Override
//            public void onClick(View v) {
//                Toast.makeText(mContext, invationInfo.toString(), Toast.LENGTH_SHORT).show();
...
//                    Toast.makeText(mContext, invationInfo.toString(), Toast.LENGTH_SHORT).show();
            ...
            hodler.reject.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    Toast.makeText(mContext, "处理了第"+position+"行", Toast.LENGTH_SHORT).show();
